...
EXTRA = {
    ...
    "the started bridge is THIS installation": (
        ...
        '    argv = ["alb"]'),
    # The guarantee is the isolation, not merely the absolute path.
    "the launch is isolated, not only the probe": (
...
def _verdict(name, mutated_source, result, failures):
    ...
    executed tests is an error for the same reason. An unnamed killer FAILS,
    because if provenance is the point then evidence we cannot attribute is
    not evidence. And a kill counts only when tests ran and something died.
    """
    try:
        compile(mutated_source, "<mutant>", "exec")
    except SyntaxError as exc:
        failures.append(f"{name}: MUTANT DOES NOT COMPILE ({exc.msg} at line "
                        f"{exc.lineno}) - no test could have judged it")
        print(f"  FAIL {name} (invalid mutant)")
        return
    stderr = result.stderr or ""
    ran = re.search(r"^Ran (\d+) test", stderr, re.MULTILINE)
    if not ran or int(ran.group(1)) == 0:
        failures.append(f"{name}: ZERO TESTS RAN - nothing judged this mutant")
        print(f"  FAIL {name} (no tests ran)")
        return
    if result.returncode == 0:
    ...
    if not killers:
        failures.append(f"{name}: KILLER UNNAMED - the gate cannot say which "
                        f"test judged this mutant")
        print(f"  FAIL {name} (killer unnamed)")
        return
    print(f"  ok   {name}  [killed by {', '.join(killers)}]")
...
def main():
    ...
    try:
        ...
        for name, (old, new) in MUTATIONS.items():
            ...
            _verdict(name, mutated, result, failures)
    finally:
